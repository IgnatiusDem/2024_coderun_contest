...
class WorkScheduleTest {
    ...
    void solutionTest() throws IOException {
        // Загрузка тестов
        String directory = "src/test/resources/demignatius/coderun/workschedule/tests/";
        TestLoader testLoader = new TestLoader();
        testLoader.loadTestsFromFile(directory);
        List<String> tests = testLoader.getTests();

        // Загрузка ответов
        directory = "src/test/resources/demignatius/coderun/workschedule/answers/";
        ...
        for (int i = 0; i < tests.size(); i++) {
            ...
            Assertions.assertEquals(answers.get(i), WorkSchedule.getLastResult());
        }
    }
}